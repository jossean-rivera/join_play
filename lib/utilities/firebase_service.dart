...
class FirebaseService {
  ...
  Future<void> registerForEvent(String eventId, String userId) async {
    try {
      final eventDocRef =
          _firestore.collection('events-collection').doc(eventId);
      ...
      await eventDocRef.update({
        'registeredUsers': FieldValue.arrayUnion(
            [userId]), // Still stored as string in events-collection
      ...
      await registrationDoc.set({
        ...
        'timestamp':
            FieldValue.serverTimestamp(), // Optional: track registration time
      });
  ...
  DocumentReference<Map<String, dynamic>> getEventDocumentReference(
      String eventId) {
  ...
  Future<void> updateEvent(String gameId, SportEvent event) async {
    try {
      ...
      final eventDocRef =
          _firestore.collection('events-collection').doc(gameId);
  ...
  Future<String?> unregisterFromEvent(String eventId, String userId) async {
    try {
      ...
      final registrationQuery = await _firestore
          ...
          .where('eventId',
              isEqualTo: eventDocRef) // Compare as DocumentReference
          .where('userId',
              isEqualTo: userDocRef) // Compare as DocumentReference
          .get();
      ...
      return 'There was an error while trying to unregister. Please try again later.';
    }
  }

  Future<List<SportEvent>> getUserRegisteredEvents(
      String userId, bool showFutureEvents) async {
    try {
      ...
      final registrationQuery = await _firestore
          .collection('registration')
          .where('userId',
              isEqualTo: userDocRef) // Use DocumentReference for userId
          .get();

      final eventDocRefs = registrationQuery.docs.map((doc) {
        final data = doc.data();
        return data['eventId']
            as DocumentReference; // Fetch as DocumentReference
      }).toList();
      ...
      final hostSnapshot = await _firestore
          ...
          .where('userId',
              isEqualTo: _firestore
                  .collection('users')
                  .doc(userId)) // Match userId as DocumentReference
          .get();

      // Extract event IDs from the 'host' collection
      final eventIds = hostSnapshot.docs
          .map((doc) {
            final data = doc.data();
